...
def alert_summary(alert):
    return {
        ...
        "candidate_entry_price": alert.get("candidate_entry_price"),
...
def intraday_timeframe_decision_for_alert(alert, payload):
    ...
    decision_use = str(matched.get("decision_use") or "").strip()
    allowed_decision_uses = {"soft_confirmation_eligible", "cap_or_challenge_only", "diagnostic_only"}
    decision_use_valid = decision_use in allowed_decision_uses
    reasons = normalize_list(matched.get("reasons"))
    if not decision_use:
        reasons.append("intraday_timeframe_decision_use_missing")
        decision_use = "diagnostic_only"
    elif not decision_use_valid:
        reasons.append("intraday_timeframe_decision_use_invalid")
        decision_use = "diagnostic_only"
    return {
        **base,
        "matched": True,
        ...
        "decision_use_present": bool(matched.get("decision_use")),
        "decision_use_valid": decision_use_valid,
        "allowed_effects": normalize_list(matched.get("allowed_effects")) if decision_use_valid else [],
...
def context_attention_items(
    ...
    timeframe_decision = intraday_timeframe_decision if isinstance(intraday_timeframe_decision, dict) else {}
    if set(timeframe_decision.get("reasons") or []) & {
        "intraday_timeframe_decision_use_missing",
        "intraday_timeframe_decision_use_invalid",
    }:
        attention.append("intraday_timeframe_decision_contract_requires_acknowledgement")
...
def position_judgment_template_for_item(item, packet_id, judgment_file):
    ...
    return {
        ...
        "draft_jsonl_object": {
            ...
            "reviewed_urgency": item.get("urgency"),
            "portfolio_id": item.get("portfolio_id"),
            "role": item.get("role"),
            "symbol": item.get("symbol"),
...
def judgment_contract(judgment_file):
    return {
        ...
        "append_jsonl_object": {
            ...
            "current_session_quote_evidence_acknowledged": "required true when alert.current_session_quote_evidence.used_in_full_score=true or alert.factor_evidence_basis.current_session_quote>0",
            "current_session_quote_evidence_basis": "required copy of alert.current_session_quote_evidence.basis when current-session quote evidence contributed to full_score",
            "current_session_quote_evidence_notes": "required notes explaining alert.current_session_quote_evidence.score_impact, whether same-session quote evidence dominated or only supported the score, and that it is provisional, can change before close, does not mutate completed daily OHLCV, and does not replace a completed daily bar",
        },
        "hard_rules": [
            ...
            "When alert.current_session_quote_evidence.used_in_full_score=true or alert.factor_evidence_basis.current_session_quote>0, do not approve/reduce unless current_session_quote_evidence_acknowledged=true, current_session_quote_evidence_basis copies the reviewed basis when present, and current_session_quote_evidence_notes discusses alert.current_session_quote_evidence.score_impact, explains whether same-session quote evidence dominated or only supported the score, and states that the quote evidence is provisional, can change before close, does not mutate completed daily OHLCV, and does not replace a completed daily bar.",
        ],
    }
...
def position_judgment_contract(judgment_file):
    return {
        ...
        "hard_rules": [
            ...
            "Copy packet_id and review_id exactly so audits can resolve the packet and position_review item reviewed; also copy review_thread_key, reviewed_recommended_action, and reviewed_urgency so fresh same-position reviews can survive packet refresh without covering escalated actions or urgency.",
