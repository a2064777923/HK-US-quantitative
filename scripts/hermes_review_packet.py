...
def judgment_contract(judgment_file):
    return {
        ...
        "append_jsonl_object": {
            ...
            "current_session_quote_evidence_notes": "required notes explaining that same-session quote evidence is provisional, can change before close, does not mutate completed daily OHLCV, and does not replace a completed daily bar",
        },
        "hard_rules": [
            ...
            "When alert.current_session_quote_evidence.used_in_full_score=true or alert.factor_evidence_basis.current_session_quote>0, do not approve/reduce unless current_session_quote_evidence_acknowledged=true, current_session_quote_evidence_basis copies the reviewed basis when present, and current_session_quote_evidence_notes explains that the quote evidence is provisional, can change before close, does not mutate completed daily OHLCV, and does not replace a completed daily bar.",
        ],
    }
