#!/usr/bin/env python3
"""現實版回測：固定本金、固定倉位、不複利、近5年"""
import csv, json, statistics, sys
from collections import defaultdict

if hasattr(sys.stdout, "reconfigure"):
    sys.stdout.reconfigure(encoding="utf-8", errors="replace")

print("載入數據...", flush=True)
sd = defaultdict(lambda: {'d':[],'o':[],'h':[],'l':[],'c':[],'v':[],'m':''})
for path, mkt in [('/tmp/hk_klines_v2.csv','HK'), ('/tmp/us_klines.csv','US')]:
    with open(path) as f:
        for row in csv.DictReader(f):
            s = row['symbol']
            if row['dt'] < '2021-01-01': continue  # 只取近5年
            sd[s]['d'].append(row['dt'])
            sd[s]['o'].append(float(row['open_price']))
            sd[s]['h'].append(float(row['high_price']))
            sd[s]['l'].append(float(row['low_price']))
            sd[s]['c'].append(float(row['close_price']))
            sd[s]['v'].append(float(row['volume']))
            sd[s]['m'] = mkt

syms = [s for s in sd if len(sd[s]['d']) >= 200]
hk = [s for s in syms if sd[s]['m']=='HK']
us = [s for s in syms if sd[s]['m']=='US']
print(f"港股{len(hk)}隻 美股{len(us)}隻", flush=True)

d2i = {sym: {sd[sym]['d'][i]:i for i in range(len(sd[sym]['d']))} for sym in syms}
all_d = sorted(set().union(*[set(sd[s]['d']) for s in syms]))

def rsi(c, p=14):
    if len(c)<p+1: return None
    g=[max(c[i]-c[i-1],0) for i in range(1,len(c))]
    l=[max(c[i-1]-c[i],0) for i in range(1,len(c))]
    ag=sum(g[:p])/p; al=sum(l[:p])/p
    for i in range(p,len(g)): ag=(ag*(p-1)+g[i])/p; al=(al*(p-1)+l[i])/p
    return 100-(100/(1+ag/al)) if al>0 else 100

def score(closes,highs,lows,vols):
    n=len(closes)
    if n<30: return None
    c=closes[-1]
    ma5=sum(closes[-5:])/5 if n>=5 else None
    ma10=sum(closes[-10:])/10 if n>=10 else None
    ma20=sum(closes[-20:])/20 if n>=20 else None
    t=0
    if ma5 and ma10 and ma20:
        if c>ma5>ma10>ma20: t=0.8
        elif c>ma5 and c>ma10: t=0.4
        elif c<ma5<ma10<ma20: t=-0.8
        elif c<ma5 and c<ma10: t=-0.4
        if n>=25:
            m=sum(closes[-25:-5])/20
            if m>0:
                s=(ma20-m)/m
                if s>0.03: t+=0.2
                elif s<-0.03: t-=0.2
    m=0; r=rsi(closes)
    if r:
        if r>70: m-=0.3
        elif r>55: m+=0.3
        elif r<30: m+=0.3
        elif r<45: m-=0.2
    if n>=35:
        def ema(d,p):
            k=2/(p+1); r=[d[0]]
            for i in range(1,len(d)): r.append(d[i]*k+r[-1]*(1-k))
            return r
        ef=ema(closes,12); es=ema(closes,26)
        ml=[ef[i]-es[i] for i in range(n)]; sl=ema(ml,9); hist=[ml[i]-sl[i] for i in range(n)]
        if hist[-1]>0 and ml[-1]>0: m+=0.3
        elif hist[-1]>0: m+=0.1
        elif hist[-1]<0 and ml[-1]<0: m-=0.3
        elif hist[-1]<0: m-=0.1
    s=0
    if n>=20:
        w=closes[-20:]; ma=sum(w)/20; std=(sum((x-ma)**2 for x in w)/20)**.5
        if c<=ma-2*std*1.02: s+=0.3
        elif c>=ma+2*std*0.98: s-=0.2
    v=0
    if n>=20:
        a20=sum(vols[-20:])/20
        if a20>0:
            vr=vols[-1]/a20
            if vr>1.5 and c>closes[-2]: v+=0.2
            elif vr>2.0: v+=0.1
            elif vr<0.5: v-=0.1
    return max(-1,min(1,t+m+s+v))

def ch_stop(h,l,c,mult=2):
    if len(c)<23: return None
    trs=[max(h[i]-l[i],abs(h[i]-c[i-1]),abs(l[i]-c[i-1])) for i in range(len(c)-22,len(c))]
    return max(h[-22:])-mult*(sum(trs)/22)

# ========== 現實參數 ==========
INITIAL = 100000.0
FIXED_TRADE_SIZE = 10000.0   # 每筆固定$1萬
MAX_CONCURRENT = 10           # 最多10筆同時持倉
BUY = 0.65; SELL = 0.35; SLIP = 0.002; CD = 3; SCAN = 5

# ========== 回測 ==========
cash = INITIAL  # 現金唔變（因為每筆固定金額）
positions = {}  # sym -> {entry_price, shares, stop_loss, entry_date, cost_basis}
cooldowns = {}
trades = []
equity_curve = []

print(f"開始回測 {all_d[0]}~{all_d[-1]}...", flush=True)

for di, date in enumerate(all_d):
    # 持倉市值
    holdings_value = 0
    for s, p in positions.items():
        if s in d2i and date in d2i[s]:
            holdings_value += sd[s]['c'][d2i[s][date]] * p['sh']
        else:
            holdings_value += p['ep'] * p['sh']
    
    total_equity = cash + holdings_value
    equity_curve.append({'d': date, 'eq': total_equity, 'cash': cash, 'hold': holdings_value, 'pos': len(positions)})
    
    # 止損
    tc = []
    for s, p in list(positions.items()):
        if s not in d2i or date not in d2i[s]: continue
        ci = d2i[s][date]
        if ci >= 22:
            hh = sd[s]['h'][max(0,ci-21):ci+1]; hl = sd[s]['l'][max(0,ci-21):ci+1]; hc = sd[s]['c'][max(0,ci-21):ci+1]
            cs = ch_stop(hh, hl, hc)
            if cs and cs > p['sl']: p['sl'] = cs
        if sd[s]['l'][ci] <= p['sl']:
            ep = max(sd[s]['o'][ci], p['sl']) * (1 - SLIP)
            pnl = (ep - p['ep']) * p['sh']
            pnl_pct = (ep / p['ep'] - 1) * 100
            cash += ep * p['sh']
            trades.append({'s': s, 'm': sd[s]['m'], 'ed': p['ed'], 'xd': date, 
                'ep': p['ep'], 'xp': round(ep, 4), 'sh': p['sh'],
                'pn': round(pnl, 2), 'pc': round(pnl_pct, 2), 'r': '止損', 'sc': p['sc']})
            tc.append(s); cooldowns[s] = date
    for s in tc: del positions[s]
    
    # 每5日掃描
    if di % SCAN == 0:
        # SELL
        tc2 = []
        for s, p in list(positions.items()):
            if s in tc: continue
            if s not in d2i or date not in d2i[s]: continue
            ci = d2i[s][date]
            sc = score(sd[s]['c'][:ci+1], sd[s]['h'][:ci+1], sd[s]['l'][:ci+1], sd[s]['v'][:ci+1])
            if sc is not None and sc <= SELL:
                ep = sd[s]['o'][ci] * (1 - SLIP)
                pnl = (ep - p['ep']) * p['sh']
                pnl_pct = (ep / p['ep'] - 1) * 100
                cash += ep * p['sh']
                trades.append({'s': s, 'm': sd[s]['m'], 'ed': p['ed'], 'xd': date,
                    'ep': p['ep'], 'xp': round(ep, 4), 'sh': p['sh'],
                    'pn': round(pnl, 2), 'pc': round(pnl_pct, 2), 'r': 'SELL', 'sc': p['sc']})
                tc2.append(s)
        for s in tc2: del positions[s]
        
        # 開倉（固定金額，唔複利）
        if len(positions) < MAX_CONCURRENT and cash >= FIXED_TRADE_SIZE:
            cands = []
            for sym in syms:
                if sym in positions: continue
                if sym in cooldowns:
                    try:
                        if (all_d.index(date) - all_d.index(cooldowns[sym])) < CD: continue
                    except: continue
                if sym not in d2i or date not in d2i[sym]: continue
                ci = d2i[sym][date]
                if ci < 60: continue
                sc = score(sd[sym]['c'][:ci+1], sd[sym]['h'][:ci+1], sd[sym]['l'][:ci+1], sd[sym]['v'][:ci+1])
                if sc is not None and sc >= BUY:
                    cands.append((sym, sc))
            cands.sort(key=lambda x: x[1], reverse=True)
            
            for sym, sc in cands:
                if len(positions) >= MAX_CONCURRENT: break
                if cash < FIXED_TRADE_SIZE: break
                ci = d2i[sym][date]
                ep = sd[sym]['o'][ci] * (1 + SLIP)
                shares = int(FIXED_TRADE_SIZE / ep)
                if shares <= 0: continue
                if sd[sym]['m'] == 'HK':
                    shares = (shares // 1000) * 1000
                    if shares <= 0: continue
                cost = ep * shares
                if cost > cash:
                    shares = int(cash / ep)
                    if sd[sym]['m'] == 'HK': shares = (shares // 1000) * 1000
                    if shares <= 0: continue
                    cost = ep * shares
                
                hh = sd[sym]['h'][max(0,ci-21):ci+1]; hl = sd[sym]['l'][max(0,ci-21):ci+1]; hc = sd[sym]['c'][max(0,ci-21):ci+1]
                cs = ch_stop(hh, hl, hc); sl = cs if cs else ep * 0.92
                positions[sym] = {'ep': ep, 'sh': shares, 'sl': sl, 'ed': date, 'sc': sc}
                cash -= cost
    
    if di % 200 == 0:
        print(f"  Day {di}/{len(all_d)}: equity={total_equity:,.0f} cash={cash:,.0f} pos={len(positions)} trades={len(trades)}", flush=True)

# 清倉
for s, p in list(positions.items()):
    if s in sd:
        lp = sd[s]['c'][-1] * (1 - SLIP)
        pnl = (lp - p['ep']) * p['sh']
        cash += lp * p['sh']
        trades.append({'s': s, 'm': sd[s]['m'], 'ed': p['ed'], 'xd': sd[s]['d'][-1],
            'ep': p['ep'], 'xp': round(lp, 4), 'sh': p['sh'],
            'pn': round(pnl, 2), 'pc': round((lp/p['ep']-1)*100, 2), 'r': '結束', 'sc': p['sc']})

# ========== 分析 ==========
final_equity = cash  # 所有持倉已清
total_return = (final_equity / INITIAL - 1) * 100
years = len(all_d) / 252

# MaxDD on equity curve
pk = 0; mdd = 0
for d in equity_curve:
    if d['eq'] > pk: pk = d['eq']
    dd = (pk - d['eq']) / pk * 100 if pk > 0 else 0
    if dd > mdd: mdd = dd

w = [t for t in trades if t['pn'] > 0]; l = [t for t in trades if t['pn'] <= 0]
wr = len(w)/len(trades)*100 if trades else 0
aw = sum(t['pc'] for t in w)/len(w) if w else 0
al = sum(t['pc'] for t in l)/len(l) if l else 0
avg_pnl = sum(t['pn'] for t in trades)/len(trades) if trades else 0
total_pnl = sum(t['pn'] for t in trades)

dr = [(equity_curve[i]['eq']/equity_curve[i-1]['eq']-1) for i in range(1, len(equity_curve))]
mn = sum(dr)/len(dr)
sd_r = (sum((r-mn)**2 for r in dr)/len(dr))**.5
sharpe = mn/sd_r*(252**.5) if sd_r > 0 else 0

# 按年份
ys = defaultdict(lambda: {'c':0,'p':0,'w':0,'hk':0,'us':0})
for t in trades:
    y = t['xd'][:4]; ys[y]['c'] += 1; ys[y]['p'] += t['pn']
    if t['pn'] > 0: ys[y]['w'] += 1
    if t['m'] == 'HK': ys[y]['hk'] += 1
    else: ys[y]['us'] += 1

# 按市場
hk_t = [t for t in trades if t['m']=='HK']; us_t = [t for t in trades if t['m']=='US']
hk_w = [t for t in hk_t if t['pn']>0]; us_w = [t for t in us_t if t['pn']>0]

cl = 0; mcl = 0
for t in trades:
    if t['pn'] <= 0: cl += 1; mcl = max(mcl, cl)
    else: cl = 0

print("\n" + "="*80)
print("📊 現實版回測結果（固定$1萬/筆、不複利、近5年）")
print("="*80)
print(f"港股: {len(hk)}隻 | 美股: {len(us)}隻 | 合計: {len(syms)}隻")
print(f"期間: {all_d[0]}~{all_d[-1]} ({len(all_d)}天, {years:.1f}年)")
print(f"初始資金: ${INITIAL:,.0f}")
print(f"最終資金: ${final_equity:,.0f}")
print(f"總回報: {total_return:.1f}%")
print(f"年化回報: {total_return/years:.1f}%")
print(f"")
print(f"📈 收益:")
print(f"  總盈虧: ${total_pnl:,.0f}")
print(f"  平均每筆: ${avg_pnl:,.0f}")
print(f"  Sharpe: {sharpe:.2f}")
print(f"")
print(f"📉 風險:")
print(f"  MaxDD: {mdd:.1f}%")
print(f"  最大連續虧損: {mcl}筆")
print(f"")
print(f"🔄 交易:")
print(f"  總計: {len(trades)}筆 | 勝率: {wr:.1f}%")
print(f"  盈+{aw:.1f}% 虧{al:.1f}% | 盈虧比 {abs(aw/al):.1f}:1" if al!=0 else "")
print(f"  期望值: {(wr/100*aw+(1-wr/100)*al):.2f}%/筆")
print(f"")
print(f"🇭🇰 港股: {len(hk_t)}筆 勝率{len(hk_w)/len(hk_t)*100:.0f}% P&L ${sum(t['pn'] for t in hk_t):,.0f}" if hk_t else "")
print(f"🇺🇸 美股: {len(us_t)}筆 勝率{len(us_w)/len(us_t)*100:.0f}% P&L ${sum(t['pn'] for t in us_t):,.0f}" if us_t else "")
print(f"\n按年份:")
for y in sorted(ys):
    s = ys[y]
    print(f"  {y}: {s['c']}筆(港{s['hk']}/美{s['us']}) 勝率{s['w']/s['c']*100:.0f}% P&L ${s['p']:,.0f}")

with open('/tmp/portfolio_bt_realistic.json', 'w') as f:
    json.dump({'summary': {'init': INITIAL, 'final': round(final_equity, 2), 'ret': round(total_return, 2), 'annual': round(total_return/years, 2), 'sharpe': round(sharpe, 2), 'dd': round(mdd, 2), 'trades': len(trades), 'wr': round(wr, 1), 'avg_pnl': round(avg_pnl, 2), 'total_pnl': round(total_pnl, 2)}, 'trades': trades, 'equity': equity_curve, 'years': {y: {'c': s['c'], 'p': round(s['p'], 2), 'wr': round(s['w']/s['c']*100, 1), 'hk': s['hk'], 'us': s['us']} for y, s in ys.items()}}, f, ensure_ascii=False, indent=2, default=str)
print("\n已存 /tmp/portfolio_bt_realistic.json", flush=True)
