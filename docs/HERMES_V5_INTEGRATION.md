...
- Confirmed BUY/SELL candidates must also satisfy the v5 risk model's minimum risk/reward ratio. The default `risk_model.min_rr_ratio=1.2` matches the order-intake minimum; lower-RR directional triggers are emitted as diagnostic `WATCH` rows with candidate risk fields, not trade candidates.
...
- `signal_id` date prefixes prefer the alert's parsed `quote_time` when available, falling back to `generated_at` only when the quote timestamp is missing or unparseable. This keeps overnight US alerts and outcome/event-store tracing aligned to the signal's market data timestamp without changing cooldown keys, order intake, Hermes judgments, or execution mode.
...
For invalid directional risk geometry or a candidate risk/reward ratio below `risk_model.min_rr_ratio`, v5 also emits the candidate as `WATCH` before it can enter trade review. `risk_geometry_valid=false` with `risk_geometry_reason` explains whether entry/stop/take-profit was missing, non-positive, geometrically invalid, or `rr_ratio_below_minimum`; executable `stop_loss`, `take_profit`, and `rr_ratio` stay empty while the `candidate_*` fields and `min_rr_ratio` remain available for diagnostics. This keeps low-price, abnormal-ATR, or poor-payoff alerts visible without polluting the BUY/SELL execution-candidate queue.
