...
- Position review items now include `advisory_plan.schema=position_advisory_plan_v1`, with `primary_action`, `add_allowed_after_review`, optional `manual_max_quantity_hint`, and reference stop/target/trailing prices. This is a Hermes/operator review aid only: quantity hints are not lot-adjusted, `submits_orders=false`, and any action still needs manual execution or a separate gated order path.
