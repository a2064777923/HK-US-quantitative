...
- `update_portfolio_prices.py` accepts `QM_PRICE_UPDATE_PORTFOLIO_ID`, so user holdings and simulation holdings can be refreshed independently. Any portfolio id present in `QM_HOLDINGS_PORTFOLIO_ID`, `QM_USER_PORTFOLIO_ID`, or `QM_USER_PORTFOLIO_IDS` is treated as user-owned and refreshes only `status='holding'` rows; if no user env is configured, portfolio `3` is the user-holding default. User refreshes never rebuild from `sim_trades` when `positions` is empty, preserving DB `positions` as the sole user-holding source of truth. Other portfolios keep the `active/holding` compatibility filter and simulation-only trade-ledger rebuild path for older simulation state. The job updates valuation fields only (`current_price`, `market_value`, unrealized PnL, portfolio totals); it does not change quantities, close/open positions, write Hermes judgments, or submit orders.
