...
- Position review items now include `advisory_plan.schema=position_advisory_plan_v1`, with `primary_action`, `add_allowed_after_review`, optional `manual_max_quantity_hint`, latest-signal geometry references, and optional trailing references. This is a Hermes/operator review aid only: quantity hints are not lot-adjusted, `submits_orders=false`, and any action still needs manual execution or a separate gated order path.
...
- Each `position_review.items[]` may include `advisory_plan.schema=position_advisory_plan_v1`. The plan translates the review action into a compact advisory draft, for example full-exit review, half-reduction review, partial profit-taking review, add permission, quantity hint, and stop/target/trailing reference prices. It is explicitly not a broker/simulation order, not lot-adjusted, and not sufficient to bypass Hermes judgment or order-intake readiness gates.
