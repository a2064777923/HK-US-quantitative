...
- `update_portfolio_prices.py` accepts `QM_PRICE_UPDATE_PORTFOLIO_ID`, so portfolio `3` user holdings and portfolio `8` simulation holdings can be refreshed independently. Portfolio `3` refreshes only `status='holding'` rows, matching the user-holding source of truth; portfolio `8` keeps `active/holding` compatibility for older simulation state. The job updates valuation fields only (`current_price`, `market_value`, unrealized PnL, portfolio totals); it does not change quantities, close/open positions, write Hermes judgments, or submit orders.
