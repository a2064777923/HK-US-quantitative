...
### 2026-06-20 factor conflict discipline

- `rt_signal_engine_v5.py` still uses current-session quote data for trigger detection, same-session momentum, intraday volume pace, realtime direction alignment, and same-market breadth. It still does not append the partial same-day quote into completed daily history.
- The live config is now `v5.8-factor-conflict-discipline-20260620`. New BUY execution candidates fail closed to diagnostic `WATCH` when same-market realtime breadth is missing, insufficient, malformed, or risk-off. Hermes should read `market_breadth_status`, `market_breadth`, `suppressed_directional_reason=market_breadth_context_missing`, and `execution_blocked_reasons` as timing/risk evidence, not as a disappeared signal.
...
- Directional execution candidates now also require opposing-factor discipline. `confirmation_requirements.*.max_opposing_factor_count=1` means a confirmed BUY or SELL candidate with more than one structured factor family supporting the opposite side is downgraded to diagnostic `WATCH` with `factor_confluence_reason=opposing_factor_conflict`. Alerts expose `factor_confluence_opposing_categories`, `factor_confluence_opposing_count`, and `factor_confluence_max_opposing_count` so Hermes can distinguish clean multi-factor agreement from mixed evidence.
...
- `update_portfolio_prices.py` accepts `QM_PRICE_UPDATE_PORTFOLIO_ID`, so user holdings and simulation holdings can be refreshed independently. Any portfolio id present in `QM_HOLDINGS_PORTFOLIO_ID`, `QM_USER_PORTFOLIO_ID`, or `QM_USER_PORTFOLIO_IDS` is treated as user-owned and refreshes only `status='holding'` rows; if no user env is configured, portfolio `3` is the user-holding default. User refreshes never rebuild from `sim_trades` when `positions` is empty, preserving DB `positions` as the sole user-holding source of truth. User price refresh prefers realtime HK/US quote endpoints before daily-kline fallback and the reviewed cron covers HK day session plus US night session. Portfolio total recomputation sums only positive-quantity open rows, matching the holding reader and preventing zero-quantity residue from inflating account value. Other portfolios keep the `active/holding` compatibility filter and simulation-only trade-ledger rebuild path for older simulation state. The job updates valuation fields only (`current_price`, `market_value`, unrealized PnL, portfolio totals); it does not change quantities, close/open positions, write Hermes judgments, or submit orders.
- Server hotfixes also aligned the container-only holding tools: `/app/scripts/read_positions.py --us` now includes `US/NASDAQ/NYSE/AMEX`, `/app/scripts/trade_update.py list` defaults to open holdings only, and legacy `smart_monitor.py` loads holdings from DB positions at startup. If DB holdings are unavailable, it now fails closed instead of falling back to stale hardcoded holdings.
- The holding tools are now tracked in GitHub as `scripts/read_positions.py`, `scripts/hk_realtime.py`, `scripts/us_realtime.py`, and `scripts/trade_update.py`. Hermes/operators should treat DB `positions` rows for portfolio `3` with `status='holding'` as the user-holding source of truth; `portfolio_report.py` uses the same user-only status filter and accepts the same `QM_HOLDINGS_PORTFOLIO_ID`/`QM_USER_PORTFOLIO_ID`/`QM_USER_PORTFOLIO_IDS` env family when building Hermes position review context. These tools do not submit orders, write Hermes judgments, mutate v5 alert state, or update the portfolio `8` simulation trade ledger.
- Server-only legacy constants such as `/root/portfolio_config.py` are not authoritative holdings configuration. The live cron/systemd paths verified on 2026-06-18 do not import it; future jobs must continue to read DB `positions` through the reviewed helpers instead of reviving that stale side channel.
...
- `trade_update.py` write commands now fail closed unless the target portfolio id appears in `QM_HOLDINGS_PORTFOLIO_ID`, `QM_USER_PORTFOLIO_ID`, or `QM_USER_PORTFOLIO_IDS`, and they still refuse configured simulation ids such as `QM_SIM_PORTFOLIO_ID=8` even if a user env var is misconfigured. This preserves no-loss integration for the existing user holding jobs while preventing accidental mutation of portfolio `8` simulation/paper state through `--portfolio-id`.
- `trade_update.py` now adjusts the internal `portfolios.available_cash` estimate by HKD trade notional by default for buy/add/sell commands, then recomputes portfolio total value from cash plus open holdings. Use `--no-cash-adjust` only when syncing a position that has already been settled in an external brokerage ledger. This is still a DB portfolio-maintenance tool, not a broker order executor or audited external fill ledger.
- `trade_update.py add` keeps the position price snapshot internally consistent: when an existing row has no usable `current_price`, the add cost is used as the refreshed quote snapshot and is written back together with market value and unrealized P&L.
- `trade_update.py delete` is no longer a normal hard-delete path. By default it soft-closes stale/invalid `holding` rows and clears open exposure fields; use `sell` for actual transactions so cash and realized P&L are updated. Physical deletion requires both `QM_ALLOW_HARD_DELETE_POSITIONS=1` and `--hard --confirm-symbol SYMBOL`, and should remain manual.
- Full user-position sells now mark the row `closed` and clear open quantity, available/frozen quantity, current cost basis, market value, unrealized PnL, PnL rate, and weight. `data_source_inventory_report.py` still counts all position rows for visibility, but `open_position_count` only includes positive-quantity `active/holding` rows, so closed SPCX/NFXS-style history cannot be misread as current exposure.
- `rt_alert_bridge.py` position-review reminders now default to one reminder per stable `role:portfolio_id:symbol` thread per 24 hours, unless urgency or recommended action escalates. The sent-state file is compacted by thread so action/date churn cannot make advisory holding reviews feel like repeated trade-operation signals.
- `simulation_postmortem_audit_report.py` and `simulation_postmortem_note_draft_report.py` now share the same dynamic postmortem-note field contract from `simulation_performance.failure_postmortem.required_learning_record.required_fields`. If simulation performance requires lineage fields such as `entry_order_id` or `signal_lineage_status`, the audit contract and Hermes-facing draft both expose them. This remains read-only: drafts keep `draft_only=true`, do not write `/tmp/simulation_postmortem_notes.jsonl`, do not submit orders, and do not promote strategy changes.
- `simulation_performance_report.py` now preserves symbol-level closed-trade lineage evidence in `closed_trade_attribution_by_symbol[]`: entry/exit order ids, per-symbol `signal_lineage_status`, and compact `closed_trade_samples[]`. Postmortem drafts copy these fields into each target so Hermes can review real losing-trade evidence instead of filling `unknown` placeholders. This only enriches read-only review context; it does not alter readiness gates, trade state, or strategy thresholds.
...
- `data_health_report.py` now emits `trade_relevant_scope`, a read-only slice for the current v5 watchlist, user portfolio holdings, and simulation holdings. Hermes can distinguish broad-universe stale/gap warnings from data issues that touch currently watched or held symbols. This context is also preserved in compact packets and source reliability coverage, but it does not override top-level `data_health.status`, execution readiness, or order-intake gates.
- After the server repair, portfolio `3` user holdings are present in Hermes as 13 DB-sourced open positions; PDD, ARAY, NOK, BABA, and MSFT use current USD quote prices instead of stale/mis-scaled DB values. Execution readiness remains `BLOCKED`; this reliability fix does not enable paper/live execution.
- `trusted_source_discovery_report.py` now treats Alpaca paper aliases (`ALPACA_API_KEY`, `ALPACA_SECRET_KEY`, `ALPACA_BASE_URL`, and the standard APCA/ALPACA variants) as broker adapter wiring, while still redacting secret values. The reviewed cron sources `/root/.quantmind_env` and `/root/.env`, so Hermes/source reliability can see configured broker adapters without changing strategy, alert queues, portfolios, or order execution mode.
...
- Position review items now include `advisory_plan.schema=position_advisory_plan_v1`, with `primary_action`, `add_allowed_after_review`, optional `manual_max_quantity_hint`, latest-signal geometry references, optional trailing references, and `dynamic_management_context`. The dynamic context carries price-snapshot age and stale flags from the DB `positions.updated_at` snapshot. This is a Hermes/operator review aid only: quantity hints are not lot-adjusted, `submits_orders=false`, and any action still needs manual execution or a separate gated order path.
- Position advisory plans avoid treating every technical SELL or shallow stop breach as a full-exit draft. Full-exit review is reserved for major loss pressure such as `<= -20%` unrealized loss, or stop breach with at least `-8%` unrealized loss. Ordinary SELL pressure or shallow stop breach maps to reduce/exit review with a partial-reduction quantity hint.
- The advisory plan's `reference_price_scope` is `latest_signal_geometry_not_position_order`. Its `signal_stop_loss` / `signal_take_profit` values are the latest signal geometry references that produced the review action; for long holdings they can sit above or below the current price and must not be confused with live broker order prices for the held position.
...
- Every v5 alert now declares its timeframe basis with `timeframe_scope=completed_daily_ohlcv_with_realtime_quote`, `primary_timeframe=1d`, `realtime_input=single_quote_temporary_bar`, `intraday_minute_bars_used=false`, and `intraday_evidence_policy=single_quote_current_session_plus_external_read_only_context`. Hermes should treat this as the technical-signal contract: v5 may use the latest realtime quote for current-session trigger, alignment, and same-session momentum evidence, but it does not consume stored minute bars directly, never mutates completed daily history, and never promotes a partial same-day bar into the completed daily series. Stored 5m/15m/30m/60m evidence must still come from the separate read-only intraday context/quality reports.
- Hermes packet eligibility now consumes that separate intraday context as a negative safety gate. If the stored same-session or multi-timeframe digest explicitly challenges the candidate side or reports conflicting timeframes, the matching trade `review_item` is rejected/held before bridge delivery or paper/simulation intake. Missing, stale, low-fidelity, or supportive-with-limits intraday context remains an acknowledgement/disclosure requirement, not a new alpha source.
- v5 alert factor contributions now carry `raw_category` and `evidence_basis` in addition to the canonical `category`. Same-session momentum still canonicalizes to `category=momentum` for factor-confluence de-duplication, but `raw_category=same_session_momentum` and `evidence_basis=current_session_quote` make it clear to Hermes that this evidence is provisional intraday quote evidence, not a completed daily momentum bar. Alert summaries also expose `factor_evidence_basis` and `current_session_quote_evidence.score_impact`, so Hermes can see exactly how many full-score points came from the current-session quote, which factor roles were affected, and whether the signal is dominated by fresh intraday movement or supported by broader completed-daily evidence. This is explanatory only; it does not relax confirmation, risk/reward, liquidity, readiness, packet-eligibility, or order-intake gates.
- v5 daily history loading excludes the quote market's current local date, so a same-day partial `interval='day'` row cannot be treated as a completed historical bar and then counted again as the realtime temporary bar. Alerts now expose `daily_history_policy=completed_daily_before_market_date`, `daily_history_cutoff_date`, `daily_history_latest_date`, and `daily_history_bar_count` so Hermes can see the exact completed-daily basis without changing alert eligibility, order intake, simulation state, or cron wiring.
- MA5 price-cross triggers compare the realtime quote against the latest completed daily MA5, not an MA5 recalculated with the same temporary quote. This prevents the quote from moving its own threshold before the cross is tested. MA10/MA20 crossover triggers compare the latest completed daily MA state with the previous completed daily MA state, so one temporary intraday quote cannot create a moving-average crossover by pulling the current MA calculation across the slower line. `MA金叉` still emits a BUY candidate and `MA死叉` still emits a SELL candidate under the same confirmation, risk/reward, cooldown, and Hermes review gates when the completed daily crossover is real.
...
- v5.8 keeps the guarded `momentum_breakout_model`. Positive `急漲` triggers at or above `large_move_buy_pct` now become BUY candidates instead of non-directional WATCH rows, but they still downgrade to WATCH unless full-score confirmation, factor confluence, opposing-factor discipline, at least two non-momentum BUY factor families for large moves, risk geometry, liquidity, realtime breadth, realtime direction alignment, and `execution_candidate` all pass. Negative large moves remain observation-only by default unless `large_move_sell_enabled=true`.
- Same-session momentum at or above `same_session_momentum_pct` adds structured BUY evidence under the canonical `momentum` factor category. This catches short-term reversal/breakout sessions that a longer bearish technical backdrop can suppress, without changing the completed-daily history contract or making stored minute bars the core scoring source. It intentionally does not count as a second independent factor when 5-day momentum is already present; same-session momentum can lift score and explain timing, but factor confluence still requires another category such as trend, RSI, MACD, Bollinger, or volume.
- For `BUY:急漲`, v5.7 goes one step stricter than generic factor confluence: at least two supporting BUY categories must be outside the canonical `momentum` family. If not, the alert stays diagnostic with `suppressed_directional_reason=large_move_non_momentum_factor_count_below_minimum` and `large_move_non_momentum_blocked=true`.
- Upper Bollinger-band breaches are now context-sensitive. Same-session or 5-day momentum can only convert an upper-band breach into `布林上軌動量突破` when at least one non-momentum BUY factor category, such as trend, RSI, MACD, volume, or broad Bollinger context, also supports the move. Momentum-only upper-band breaches remain the legacy `布林上軌突破` SELL candidate, preventing one evidence family from being counted as both trigger and confluence. A sufficiently broad BUY setup can still qualify through the stricter `bollinger_buy_min_score` plus `bollinger_buy_min_supporting_factors` branch. Hermes should inspect `trigger`, `candidate_signal_type`, `factor_contributions`, and `execution_blocked_reasons` instead of assuming every upper-band breach is mean-reversion SELL.
- Confirmed BUY/SELL candidates must also have factor confluence before they can remain executable. By default, v5 requires at least two supporting factor categories for each side, derived from structured `factor_contributions` categories such as trend, RSI, MACD, Bollinger, volume, and momentum. A high score backed by only one category is downgraded to diagnostic `WATCH` with `factor_confluence_valid=false`, `factor_confluence_categories`, `factor_confluence_supporting_count`, and `factor_confluence_reason=supporting_factor_count_below_minimum`. A candidate with more than one opposing category is also downgraded with `factor_confluence_reason=opposing_factor_conflict`. Operators may raise `confirmation_requirements.*.min_supporting_factor_count` or lower `confirmation_requirements.*.max_opposing_factor_count` for stricter behavior, but v5 will not accept supporting-factor requirements below the default of 2 or opposing-factor limits above the default of 1.
- `alert_quality_report.py` now separates live order-intake validation from strategy-quality validation. `alert_too_old` remains a hard live-intake rejection in `rt_order_intake.py`, but alert quality excludes that age-only reason from `validation_pass_rate_pct` so Hermes does not mistake stale queue inspection for weak factor/threshold logic. The report keeps both views: `live_validation_pass_rate_pct` and `live_validation_fail_reasons` for execution freshness, plus `validation_pass_rate_pct` and `quality_validation_fail_reasons` for strategy review. Its current-sample scope also follows `candidate_signal_type`, so downgraded directional `WATCH` rows still anchor the latest strategy/watchlist cohort correctly.
- A WATCH-heavy alert queue is now recorded as `diagnostic_notes[]` unless WATCH rows actually enter Hermes `review_items`. This keeps research/diagnostic rows visible for strategy analysis while preventing Hermes/operator action queues from treating filtered diagnostic WATCH volume as an actionable packet-filter defect.
...
- `execution_candidate=true` is now explicitly reserved for alerts that remain BUY/SELL after policy downgrades, are full-score confirmed, and have valid risk geometry. Hermes and `rt_order_intake.py` should not infer executability from `candidate_signal_type`, `signal_type`, or `confirmed` alone; diagnostic WATCH rows may still carry candidate risk fields for review, but they are not executable candidates. Order intake rejects `BUY`/`SELL` payloads with missing or false `execution_candidate`.
- Non-executable directional research rows now keep executable price fields empty unless `execution_candidate=true`. This includes shadow-only rows, invalid-risk rows, downgraded WATCH diagnostics, and explicit research runs that preserve unconfirmed BUY/SELL output with `RT_SIGNAL_EMIT_UNCONFIRMED_DIRECTIONAL_AS_WATCH=0`. Hermes still receives `candidate_entry_price`, `candidate_stop_loss`, `candidate_take_profit`, `candidate_rr_ratio`, and `execution_blocked_reasons`, so candidate geometry remains available without making a research row look tradable.
- `rt_signal_outcome_report.py` now emits an execution-candidate-only cohort under `execution_candidate`, including `overall` and `by_trigger`, whenever downgraded diagnostic directional rows are present. `strategy_review_report.py` reads that cohort for policy decisions when it exists, so same-scan duplicate WATCH diagnostics stay visible for Hermes but do not inflate trigger promotion evidence.
- `hermes_review_packet.py` now points `strategy_learning_brief.outcome_evidence` at the executable-only learning cohort when that cohort is present, and keeps the all-candidate summary separately as diagnostic context. Hermes should treat the executable cohort as the main evidence source for strategy promotion review whenever diagnostic directional rows exist.
- `strategy_learning_report.py` also separates executable trigger learning as `execution_candidate_by_trigger`. When downgraded diagnostic directional rows exist, trigger weakness recommendations such as `trigger_forward_return_non_positive:*` are derived from the executable cohort instead of all-candidate `by_trigger`, so duplicate or shadow diagnostics cannot drive strategy rework advice by themselves.
...
- The latest alert file and append-only alert queue are both written as UTF-8 JSON. This keeps Chinese trigger names and `full_reasons` stable for Hermes and Feishu readers without changing the alert schema, queue path, execution mode, or order intake contract.
...
The bridge treats packet freshness as eligibility-critical context. `RT_ALERT_MAX_PACKET_CONTEXT_AGE_MINUTES=10` by default. If the packet is stale, missing, or has an invalid/missing/future `generated_at`, the alert is excluded from actionable output, diagnostic output says `Hermes審核狀態：STALE`, `MISSING`, or `INVALID`, and `alert-sim` does not call order intake for that alert. If a fresh packet exists but the alert's `signal_id` is not in it, the notification says `Hermes審核：NO_MATCH` and marks the alert as a technical signal that has not completed comprehensive review. These warnings do not submit orders, change simulation state, or mutate any DB rows. They prevent stale, missing, invalid, or unmatched packet context from looking like Hermes-approved market/news/intraday evidence while preserving notify-only behavior.
...
Order intake evaluates aggregate readiness and strategy evidence after broker context, existing-position context, order-plan construction, and Hermes trade judgment. This preserves the normal fail-closed path for new `BUY` exposure, but lets an already-held simulation/paper position receive a reviewed risk-reduction `SELL` exit or reduction when the only blockers are bootstrap/attribution blockers such as missing forward outcome samples, low directional intake coverage, missing Hermes judgment-effect samples, or failed simulation performance/trade-review evidence. The override is exposed as `risk_reduction_override` and requires `plan.side=sell`, `risk_hkd=0`, an existing broker/simulation position for the symbol, and a fresh Hermes `approve` or `reduce` judgment. It does not override system/data/source health blockers, stale or invalid readiness reports, explicit negative strategy outcome evidence, market/broker/pilot/auth gates, or any new `BUY`.
...
The packet gate is intentionally separate from the judgment gate. It requires a fresh `hermes_signal_review_packet_v1`, a matching `review_items[].signal_id`, and `eligible_for_approval=true` before a judgment can be used for execution. The matching trade judgment must also copy that same packet's `packet_id`; execute mode rejects missing or mismatched judgment packet IDs before submission so an old approval cannot outlive a newer packet-level blocker. New judgments should also copy `reviewed_symbol`, `reviewed_signal_type`, and `reviewed_trigger` from the reviewed packet alert. These identity fields are backward-compatible: old judgments without them are still evaluated by `signal_id` and `packet_id`, but if Hermes writes them and they do not match the alert being submitted, `rt_order_intake.py` rejects execute mode and `hermes_judgment_audit_report.py` flags the judgment. `RT_ORDER_MAX_HERMES_PACKET_AGE_MINUTES` defaults to `30`. Missing, stale, invalid, unmatched, or ineligible packets reject execute mode with `hermes_packet_gate_failed`; missing or mismatched judgment packet IDs or reviewed-alert identity mismatches reject with `hermes_judgment_gate_failed`. Dry-run output reports `hermes_packet.would_block_execute=true` without submitting. Existing bridge jobs can keep generating the same packet every minute; no alert schema migration is required.

Execute-mode rejections caused by missing/invalid/stale/unmatched Hermes packet data or missing/mismatched/expired Hermes judgments are retryable metadata failures. `rt_order_intake.py` records those rejected attempts under `dry_runs` instead of `processed`, so the same `signal_id` can be retried after Hermes packet or judgment repair. Actual submissions, broker/pilot/readiness/strategy/market rejections, and explicit ineligible/reject/hold judgments still write `processed` and remain idempotent.
...
```json
{
  ...
  "reviewed_symbol": "00700",
  "reviewed_signal_type": "BUY",
  "reviewed_trigger": "站上MA5",
  "decision": "approve",
  "confidence": 0.74,
...
`hermes_review_packet.py` exposes the same required `context_review` checklist in top-level `judgment_contract.append_jsonl_object.context_review`, including `intraday_context_reviewed`, and it includes `reviewed_symbol`, `reviewed_signal_type`, and `reviewed_trigger` identity-copy fields beside `signal_id` and `packet_id`. Tests compare that packet contract directly with `hermes_judgment_audit_report.py::REQUIRED_CONTEXT_REVIEW_FLAGS`, so Hermes should not receive a stale judgment template that later fails audit. The packet contract also lists structured intraday acknowledgement fields (`intraday_context_acknowledged`, `intraday_context_status`, `intraday_context_notes`) for cases where `context_digest.required_judgment_attention` raises intraday missing/stale, contradiction, or timeframe-coverage-limit items, plus the finer `intraday_signal_evidence_*` fields when 5m/15m/30m/60m/session evidence supports, challenges, conflicts, or has quality/source limits.

Each `review_items[].signal_id` in a Hermes review packet must be unique. Duplicate packet rows for the same signal fail closed with `hermes_packet_duplicate_review_items` in order intake and `packet_duplicate_review_items` in the judgment audit, because execute and audit cannot know which row Hermes reviewed.
...
`review_items[].context_digest.intraday_signal_evidence` is the compact per-signal 5m/15m/30m/60m/session evidence contract. It maps the raw minute digest into `alignment=supports_signal|supports_with_limits|challenges_signal|conflicting_timeframes|limited_context|unavailable_or_stale|neutral_or_insufficient`, with `support_codes`, `challenge_codes`, `conflict_codes`, `quality_codes`, `limit_codes`, and a flattened `codes[]`. When v5 downgrades a directional BUY/SELL candidate to emitted `WATCH`, this evidence reviews the original `candidate_signal_type` and also records `emitted_signal_type`, so Hermes answers whether intraday context supports or challenges the real candidate idea without treating the WATCH row as executable. When `requires_judgment_acknowledgement=true`, Hermes must not approve or reduce unless the judgment includes:
...
When `alert.current_session_quote_evidence.score_impact.factor_count > 0`, `current_session_quote_evidence_notes[]` must discuss that score impact and whether the same-session quote evidence dominated the signal or only supported broader evidence. A generic note that the quote is provisional is no longer enough; `hermes_judgment_audit_report.py` fails it with `current_session_quote_score_impact_notes_missing`. This preserves the live-market freshness layer while forcing Hermes to critique whether a BUY/SELL is being carried by a short-lived intraday move.
...
When v5 downgrades a directional candidate to `WATCH`, Hermes packet statistics use `candidate_signal_type` for directional counts while preserving emitted `signal_type` in `by_signal_type`. Use `alert_selection.downgraded_directional_count` and `by_candidate_signal_type` to see how much BUY/SELL evidence was suppressed by shadow, disabled-rework, confirmation, or risk gates without routing those rows into approval review.

`execution_candidate=false` directional alerts are also excluded from trade review and moved into `non_actionable_observations` as research-only evidence. The packet uses the alert's execution contract first, so even if a downstream intake result is incomplete or inconsistent, a non-executable BUY/SELL alert stays out of `review_items[]`. The packet still counts them separately in `alert_selection.execution_candidate_directional_count` and `alert_selection.non_execution_candidate_directional_count`, so Hermes can see how many directional rows were executable versus only diagnostic without accidentally treating them as trade judgments.
...
- Each `position_review.items[]` may include `advisory_plan.schema=position_advisory_plan_v1`. The plan translates the review action into a compact advisory draft, for example full-exit review, half-reduction review, partial profit-taking review, add permission, quantity hint, stop/target/trailing reference prices, and `dynamic_management_context` with target status, stop/target distance, latest daily move, P&L, price snapshot age/freshness, and dynamic review focus. It is explicitly not a broker/simulation order, not lot-adjusted, and not sufficient to bypass Hermes judgment or order-intake readiness gates.
...
- Holdings with latest daily moves also create advisory review tasks even when no v4 signal exists. A daily gain at or above `+3%` becomes `take_profit_or_trailing_stop_review`; a daily loss at or below `-3%` becomes `reduce_or_exit_review`. `dynamic_management_context` then tells Hermes whether the holding is below signal stop, near/at signal target, in profit momentum review, or in downside review, and whether the DB price snapshot is fresh enough to trust without disclosure. Target extension, partial take-profit, trailing floor, add, or reduce advice can then be discussed without mutating orders or positions. This is how user holdings such as `SPCX` can be surfaced for trailing-stop/target adjustment even when `engine_signal_scores` has no row for that symbol.
...
- `/root/update_portfolio_prices.py` every 15 minutes across HK day session and US night session for user holdings; the simulation snapshot line remains scoped to the existing HK-hours compatibility path.
...
- `rt_alert_bridge.py` includes high and medium urgency `role=user` position reviews by default and caps the queue at 20 items. Simulation holdings remain available in Hermes packets and reports, but they are not mixed into operator Feishu reminders unless `RT_POSITION_REVIEW_ROLES=simulation` or `all` is set explicitly.
...
This mode does not touch the simulation trading system. It only emits formatted Hermes text. When `/tmp/hermes_signal_review_packet.json` is present and fresh, the text includes the matching review item's `eligible_for_approval`, `recommended_judgment`, compact `blocking_reasons`, top-level `simulation_performance.status` and reason codes, top-level `strategy_learning_brief.hermes_alpha_evidence`, top-level `execution_readiness.status/ready_for_execute` with compact blocking or warning gate names, plus the matching `context_digest`. The Feishu/Hermes-facing alert therefore shows whether the technical signal is currently reviewable or should be held/rejected, whether Hermes approval has enough realized alpha evidence, whether the aggregate system is ready for execute mode, and whether the signal is supported or contradicted by market regime, intraday data, news/events, direct event-review signals, sentiment, fundamentals, simulation feedback, and source-quality warnings. Direct event-review signals are shown as `事件審核：CHALLENGE_BUY_REVIEW:...` or `事件審核：SUPPORT_BUY_REVIEW:...` when the packet has `event_catalyst_signals.signals[]` matched to the alert by `related_v5_signal_ids[]`. If external/news, event-catalyst, event-review-signal, sentiment, or fundamentals context has no matched detail rows but its `status` is not clean, the notification still shows compact coverage lines such as `新聞/宏觀覆蓋：STALE`, `事件覆蓋：MISSING`, `事件審核覆蓋：RISK`, `情緒覆蓋：RISK`, or `基本面覆蓋：STALE`; missing rows are therefore not presented as neutral evidence. Intraday signal evidence is shown as `分鐘證據：supports_with_limits side=BUY codes=... ack=required` or, for downgraded directional rows, `分鐘證據：challenges_signal candidate=BUY emitted=WATCH codes=... ack=required`, using `context_digest.intraday_signal_evidence` so Hermes/operator can see whether 5m/15m/30m/60m/session context supports, challenges, conflicts, or has quality/source limits without opening the packet. Source reliability is shown as `來源可靠性：DEGRADED component:reason...`, using compact problem components from `context_digest.source_limits` so public-fallback, partial-fundamentals, preflight, or provenance weaknesses are visible in the Feishu alert. `Hermes Alpha：INSUFFICIENT/NEGATIVE/SUPPORTIVE`, `執行準備：READY/WARN/BLOCKED`, coverage lines, `來源可靠性：...`, and `分鐘證據：...` are display-only evidence from the packet; they do not change alert eligibility, submit orders, mutate simulation state, or promote strategy settings. If the packet is stale, missing, invalid, or has no matching `signal_id`, the bridge downgrades the wording to `Hermes審核狀態：STALE/MISSING/INVALID` or `Hermes審核：NO_MATCH` and explicitly says the alert is only a technical signal until manual/Hermes review catches up. Hermes must not treat those downgraded notifications as approved comprehensive judgments.

If the packet contains duplicate `review_items[].signal_id` rows for the same alert, `rt_alert_bridge.py` follows the same fail-closed contract as order intake: the alert is excluded from actionable output, diagnostic output shows `Hermes審核：DUPLICATE_REVIEW_ITEMS`, and `alert-sim` does not call order intake for that alert.

Bridge alert sent-state is also retry-aware for structural packet failures. Missing/invalid/stale Hermes packets, missing matching review items, and duplicate `review_items[].signal_id` rows are not written to `/tmp/rt_signal_sent.json`, even when `RT_ALERT_MARK_INELIGIBLE_SENT=1`, so a repaired packet can expose the same alert again. Ordinary safety-gate blocks such as readiness not READY, `eligible_for_approval=false`, or Hermes blocking reasons still follow the existing anti-spam behavior.

The bridge also emits advisory-only position-review notifications when `RT_ALERT_INCLUDE_POSITION_REVIEW=1` (default) and the latest packet has unsent `role=user` high/medium urgency `position_review.items[]`. This works even when there are no new actionable BUY/SELL alerts, so high-risk user holdings are not hidden behind an empty signal queue. Simulation holdings are still present in the packet/report context for Hermes analysis, but the operator-facing stream filters them out by default via `RT_POSITION_REVIEW_ROLES=user`; set `RT_POSITION_REVIEW_ROLES=simulation` or `all` only when explicitly reviewing the simulation portfolio. The bridge reads `position_review.items[].review_id`, `role`, `urgency`, `recommended_action`, `execution_policy`, position PnL/stop distance, latest signal, `advisory_plan`, `context_digest.position_attention[]`, and `position_judgment_audit.coverage.unjudged_high_urgency_review_count`, then prints a `Hermes持倉審核待辦（不下單）` section for Feishu/Hermes. The `審核草案` line is a structured review aid with candidate action, add permission, quantity hint, and `sig_stop_ref` / `sig_target_ref` latest-signal geometry references; it is not an order ticket and does not change `order_submission=false`. Each item includes `審核ID：review_id=... judgment_file=/tmp/hermes_position_judgments.jsonl` so Hermes can copy the exact key required by `hermes_position_judgment_audit_report.py` without opening the raw packet. It also prints audit-facing requirements: the five required `context_review` flags, every `position_attention` code that must be copied into `position_attention_codes[]`, `position_attention_acknowledged=true`, `position_attention_effects[]=one_per_code`, and the allowed advisory-only decisions `hold|watch|reduce|exit|trail_stop`. It writes only a separate dedupe file, `/tmp/rt_position_review_sent.json` by default, compacted by stable `role:portfolio_id:symbol` review thread; it does not append to the alert sent state, call `rt_order_intake.py`, submit orders, change portfolio rows, change simulation state, or write Hermes position judgments. If a holding thread remains unjudged, the bridge reminds after `RT_POSITION_REVIEW_REMINDER_HOURS=24` by default, unless the item's urgency or recommended action escalates first. This prevents one missed Feishu delivery from permanently hiding an unresolved high-risk holding while avoiding every-minute spam. Operators can set `RT_POSITION_REVIEW_URGENCY=high,medium`, `RT_POSITION_REVIEW_ROLES=user,simulation`, `RT_POSITION_REVIEW_LIMIT=3`, and `RT_POSITION_REVIEW_REMINDER_HOURS=24` to widen, cap, or retime the advisory queue. `RT_ALERT_INCLUDE_POSITION_REVIEW=0` restores the old alert-only bridge behavior without affecting packet generation or position judgment audit reports.
...
- `data_health_report.py` emits `trade_relevant_scope` with schema `data_health_trade_relevant_scope_v1`. It recomputes the same market coverage/integrity/source-quality checks for three smaller scopes: the current v5 watchlist from `RT_SIGNAL_WATCHLIST_FILE`, user open holdings from `QM_HOLDINGS_PORTFOLIO_ID` / `QM_USER_PORTFOLIO_ID` / `QM_USER_PORTFOLIO_IDS`, and simulation open holdings from `QM_SIM_PORTFOLIO_ID`. This lets Hermes say, for example, that broad HK universe coverage is WARN while current US watchlist/user holdings are OK, or that a user HK holding is affected by repair-source provenance. The scope is diagnostic only: it does not lower top-level `data_health.status`, does not bypass `execution_readiness_report.py`, does not change alert eligibility, does not repair data, and does not submit orders.
- `hermes_review_packet.py` preserves `data_health.trade_relevant_scope` in compact packets. `source_reliability_report.py` also copies it into the data-health component coverage so source review can distinguish broad-universe degradation from trade-relevant degradation without recalculating the scope.
...
- Each `review_items[].context_digest` also includes `intraday_timeframe_decision`, matched to the reviewed signal symbol from `intraday_timeframe_quality.markets[].symbols[]`. It copies `decision_use`, `allowed_effects`, limited/missing timeframes, reasons, alignment, and compact quality counts so Hermes can see whether this signal's 5m/15m/30m/60m evidence is `soft_confirmation_eligible`, `cap_or_challenge_only`, or `diagnostic_only` without searching the top-level report. If a matched symbol row lacks a valid `decision_use`, the packet marks the digest `diagnostic_only`, clears `allowed_effects`, adds `intraday_timeframe_decision_use_missing` or `intraday_timeframe_decision_use_invalid`, and requires acknowledgement. This digest is read-only and does not relax the global timeframe policy, daily gates, readiness, source reliability, or order intake.
...
- `system_health_check.py` emits `schema=quantmind_system_health_v1`. `execution_readiness_report.py` requires this schema before trusting the system-health status: `OK` passes, `WARN` is a warning gate, and missing/invalid schema or `FAIL` hard-blocks readiness. This separates report-contract failures from degraded-but-readable health context.
...
Even in that explicit research mode, unconfirmed BUY/SELL rows are not executable: `execution_candidate=false`, `execution_blocked_reasons` includes `not_confirmed`, and `entry_price`, `stop_loss`, `take_profit`, and `rr_ratio` stay empty. The old risk geometry remains available only under `candidate_entry_price`, `candidate_stop_loss`, `candidate_take_profit`, and `candidate_rr_ratio`. This is a no-loss integration change for Hermes because the review packet also copies these candidate fields into `review_items[].alert`, while order intake continues to reject the row before any simulation order can be created.
...
For trigger overrides with `enabled=false` and `review_mode=disabled_pending_rework`, v5 preserves a diagnostic `WATCH` row instead of dropping the trigger entirely. The original BUY/SELL remains in `candidate_signal_type`, `trigger_review_mode=disabled_pending_rework`, `strategy_policy_disabled_observation=true`, `suppressed_directional_reason=strategy_review_disabled_pending_rework`, and `execution_blocked_reasons` includes `strategy_review_disabled_pending_rework`. This keeps forward observation evidence available for later rework, while executable price fields stay empty and `execution_candidate=false`, so Hermes can critique the trigger without approving it.
...
`scripts/operator_action_queue_report.py` is a read-only priority layer for Hermes/operator remediation. It reads the current execution readiness report, cron audit, cron promotion dry-run, Hermes packet, compact position-judgment write packet, position-judgment audit, source reliability, simulation performance, and signal outcome reports, then emits `operator_action_queue_report_v1`.
...
`hermes_review_packet.py` reads `/tmp/operator_action_queue_report.json` into top-level `operator_action_queue` when available. Hermes should use it as a remediation checklist, not as trade evidence and not as execution permission. A queue item that says `writes_judgments=true` means Hermes must write completed advisory JSONL itself; templates are not judgments. For `write_high_urgency_position_judgments`, `evidence.position_judgment_write_plan[]` is a compact pointer list with exact `review_id`, thread key, allowed decisions, required attention codes, dynamic-management status, and price freshness; it helps Hermes find the right packet item/template but is still not a judgment. `hermes_position_judgment_write_packet.py` is the corresponding compact read-only draft packet: it filters the high-urgency worklist into a Hermes-friendly write queue, but it is also not a judgment and must not be appended as-is. For `write_or_repair_simulation_postmortem_notes`, `evidence.postmortem_note_write_plan[]` gives each required target id, draft availability, context statuses, target evidence, and matched context ids so Hermes can complete loss-review notes without searching the full draft report; draft placeholders must still be replaced and `draft_only` removed before appending. A queue item that says `changes_crontab=true` means the listed command still requires an explicit operator action and normal post-apply verification.
...
`execution_readiness_would_block_execute` is not a strategy weakness by itself. It means the signal or dry-run decision sits inside a broader system state that would still block `rt_order_intake.py --mode execute` until the aggregate readiness report becomes `READY`. Hermes should treat it as global execution-state context, not as forward-alpha evidence for or against the signal.
...
For `intraday_timeframe_quality` source-reliability components, `hermes_review_packet.py` also carries compact `intraday_timeframe_decision_use` counts into `context_digest.source_limits.source_reliability_problem_components[]`. Hermes can therefore see whether degraded 5m/15m/30m/60m evidence has no soft-confirmation-eligible symbols, diagnostic-only symbols, or missing decision-use counts without opening the full source-reliability report.
...
For non-executable v5 rows, `hermes_review_packet.py` preserves the same separation in `review_items[].alert`: executable `entry_price`, `stop_loss`, `take_profit`, and `rr_ratio` are populated only for `execution_candidate=true`, while non-executable research or diagnostic rows expose `candidate_*` geometry plus `execution_blocked_reasons`. Hermes should cite the blocked reason and candidate geometry as review context, not as permission to approve or route an order.

`rt_order_intake.py` also preserves that boundary when reading forward outcome evidence. If `/tmp/rt_signal_outcome_report.json` contains downgraded directional diagnostics, detected by `downgraded_directional_alert_count` or `counts.downgraded_directional_alert_count`, execute mode no longer passes from mixed `overall` or mixed `by_trigger` metrics alone. It requires an explicit executable-only cohort such as `execution_candidate.overall` plus `execution_candidate.by_trigger`, or the equivalent top-level executable-only fields. Without that cohort the strategy gate returns `strategy_evidence_includes_diagnostic_candidates_without_executable_cohort` and keeps the all-candidate metrics in the payload only as diagnostics. `execution_readiness_report.py` applies the same boundary to the dashboard's `forward_outcome_evidence` gate: mixed all-candidate outcomes cannot make readiness `READY` while downgraded diagnostics are present unless an executable-only cohort is available and passes the normal outcome thresholds. This is a no-loss integration change: older reports with no downgraded diagnostics keep the previous metric path, while newer richer reports can add executable-only cohorts without changing alert, judgment, or simulation schemas.
...
The traceability section has schema `simulation_closed_trade_signal_traceability_v1`. It reads only `/tmp/rt_order_intake_state.json` and matches FIFO closed-trade entry/exit broker IDs or client-order IDs to processed intake decisions with `order_result.order_id`, `id`, `client_order_id`, or `clientOrderId`. This covers QuantMind simulation order IDs and Alpaca paper `client_order_id` lineage. If recent closed trades exist but processed decisions are empty, the state file is missing, entry IDs are absent, or entry IDs do not match any processed signal, the report adds `closed_trade_signal_traceability_missing` and fails closed. Positive P&L without this linkage is not treated as upgrade evidence, because Hermes cannot prove which v5 signal, score, trigger, market context, source-quality state, or judgment produced the trade.
...
The report also emits `pilot_readiness` with `schema=execution_readiness_pilot_gate_v1`. This field is advisory operator context only: it separates hard safety blockers from bootstrap evidence blockers such as missing forward-outcome samples, missing Hermes judgment-effect samples, or missing v5/Hermes-traceable paper/simulation lineage. `pilot_readiness.paper_pilot_reviewable=true` never changes `ready_for_execute`, never enables `alert-sim`, never submits orders, and is not consumed by `rt_order_intake.py` as a new-exposure execution bypass; any evidence pilot still needs separate manual enablement, explicit capped paper/simulation controls, Hermes judgment, packet eligibility, market context, and broker reconciliation. The only order-intake exception is the audited risk-reduction `SELL` path for existing simulation/paper holdings described above; it does not approve new exposure or make a blocked readiness report globally executable.
...
- after a promoted config is loaded by `rt_signal_engine_v5.py`, `enabled=false` with `review_mode=disabled_pending_rework` is also WATCH-only diagnostic output. Plain `enabled=false` without that review mode remains fully silent, so operators can still suppress a trigger when observation is not wanted.
...
- `local_backtest_dataset.py` is now the local-only research data entry point for reproducible HK/US backtests. It writes `/tmp/hk_klines_v2.csv`, `/tmp/us_klines.csv`, `/tmp/all_klines.csv`, and `/tmp/hk_us_dataset_metadata.json` from Tencent HK daily bars plus Alpaca US bars. The metadata now embeds per-market manifests with concrete CSV paths, row/symbol coverage, freshness, and SHA-256 checksums so the raw lake can be verified locally without uploading it. Raw CSV/bar files are intentionally local-only: do not commit them to GitHub, do not copy them to the production server by default, and do not treat local research data as a live execution gate. Hermes can use the metadata and resulting backtest JSON as research evidence only; this does not change v5 alert generation, order intake, simulation execution, cron wiring, or live data-health authority.
- Broad and fine-grained local data collection is encouraged for research, replay, and later feature engineering, including extra symbols, hourly bars, minute bars, vendor snapshots, and provider metadata. Keep the raw lake local by default and publish only compact manifests, coverage/freshness/gap/source-quality reports, checksums, and replay/backtest summaries to Hermes, GitHub, or the production server.
- `local_backtest_reliability_report.py` summarizes those local-only dataset/backtest artifacts into `/tmp/local_backtest_reliability_report.json`. Hermes may use it to inspect breadth, drawdown, win rate, and annual consistency, but only as research evidence. It does not promote strategy configs, alter v5 thresholds, touch order intake, mutate simulation state, or replace live data-health/readiness gates.
- `factor_contract_alignment_report.py` compares the current v5 factor contract against the local portfolio backtests and writes `/tmp/factor_contract_alignment_report.json`. Hermes packet exposes this as read-only alignment context, so Hermes can see when backtests are useful baseline research but not direct v5 replay proof. It does not change v5 scoring, thresholds, watchlists, order intake, simulation state, or cron wiring.
- The alignment report now also checks whether local backtests model the v5 factor-confluence contract: structured `factor_contributions`, the minimum supporting factor-category gate, and the v5.8 opposing-factor conflict gate. `factor_confluence_contract_drift` means a backtest may share factor families and score thresholds with v5 but still overstate executable signal quality because it does not downgrade one-factor high-score setups or mixed-evidence setups with too many opposing factor families to diagnostic `WATCH`.
- Hermes packet copies alignment drift into `strategy_learning_brief.factor_contract_alignment.contract_drift_summary`, including compact `missing_from_backtest` lists, so Hermes can see factor-confluence or risk/execution contract gaps without scanning the full alignment report.
- `v5_local_replay_report.py` replays the current v5 `IncrementalIndicators + TriggerEngine` semantics and writes `/tmp/v5_local_replay_report.json`. It supports local CSV mode and a server DB snapshot mode: `/root/v5_local_replay_report.py --source db --db-lookback-days 365 --output /tmp/v5_local_replay_report.json --text`. DB mode reads existing completed daily `klines` rows for the live v5 watchlist plus DB user-holdings overlay, uses yesterday as the default end date unless `--end-date` is explicitly supplied, and does not copy raw CSV/minute data to the server. Both modes use prior completed bars plus one synthetic close-time quote per daily row, so replay is closer to v5 trigger/confirmation/risk behavior than the portfolio backtests, but it is still not a true intraday path or PnL backtest. The report includes `replay_quality` metrics for alert density, execution-candidate density, directional downgrade ratio, and same-symbol same-day trigger stacking, plus `replay_breakdown` rows by market, trigger group, structured factor category, and gate-blocker reason so Hermes can see which triggers or factor categories are noisy and which confirmation/risk/readiness gates are doing the blocking before any threshold proposal. Hermes packet exposes this as top-level `v5_local_replay` and compact `strategy_learning_brief.v5_local_replay`. Missing replay output is treated as optional `MISSING` context; present output is read-only research evidence only. This is a no-loss integration: it does not write `/tmp/rt_signal_alerts.jsonl`, submit orders, change v5, change order intake, touch simulation state, copy raw CSVs to GitHub/server, or alter any existing cron job.
- `v5_replay_strategy_review_report.py` consumes `/tmp/v5_local_replay_report.json` and writes `/tmp/v5_replay_strategy_review_report.json`. It converts replay noise into Hermes-readable trigger policy hints using the existing strategy-review vocabulary (`tighten_thresholds`, `shadow_only`, `candidate_allow_after_other_gates`) while marking every row `promotion_eligible=false`. It also attaches compact gate-blocker context from replay, so Hermes can distinguish triggers that are noisy because many candidates are already being rejected by confirmation/factor/risk gates from triggers that still produce too many execution candidates. It intentionally exposes `replay_trigger_policies`, not the standard `trigger_policies` consumed by `strategy_config_proposal.py`, so accidental wiring cannot promote replay-only hints. Hermes packet exposes this as top-level `v5_replay_strategy_review` and compact `strategy_learning_brief.v5_replay_strategy_review`. Missing output is optional `MISSING` context. This is deliberately not the same as `strategy_review_report.py`: it is not forward outcome evidence, does not feed `strategy_config_proposal.py`, does not promote thresholds, does not write alerts, does not submit orders, does not alter simulation state, and must be used only to challenge or cap Hermes confidence until forward outcomes and simulation evidence support a real strategy proposal.
- `trigger_evidence_convergence_report.py` consumes `/tmp/strategy_review_report.json` and `/tmp/v5_replay_strategy_review_report.json`, then writes `/tmp/trigger_evidence_convergence_report.json`. It compares forward-outcome policy with replay-derived noise policy by trigger and classifies `CONVERGED_RISK`, `REPLAY_CHALLENGES_FORWARD`, `FORWARD_CHALLENGES_REPLAY`, `INSUFFICIENT_FORWARD_SAMPLE`, or `CONVERGED_CLEAN`, while preserving compact replay gate-blocker counts for Hermes review. Hermes packet exposes this as top-level `trigger_evidence_convergence` and compact `strategy_learning_brief.trigger_evidence_convergence`. This is also no-loss: it is read-only, does not expose standard `trigger_policies`, does not feed `strategy_config_proposal.py`, does not change v5 thresholds, does not submit orders, and only tells Hermes whether replay and forward evidence agree, conflict, or lack sample.
- `strategy_config_proposal.py` and `operator_action_queue_report.py` preserve those compact replay gate-blocker counts inside convergence risk context and operator review actions. This gives Hermes/operators the reason a replay trigger is risky without changing promotion blockers, auto-applying strategy changes, or weakening execution gates.
