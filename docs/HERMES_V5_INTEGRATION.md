...
- The alignment report now also checks whether local backtests model the v5 factor-confluence contract: structured `factor_contributions` plus the minimum supporting factor-category gate. `factor_confluence_contract_drift` means a backtest may share factor families and score thresholds with v5 but still overstate executable signal quality because it does not downgrade one-factor high-score setups to diagnostic `WATCH`.
