...
Bridge alert sent-state is also retry-aware for structural packet failures. Missing/invalid Hermes packets, missing matching review items, and duplicate `review_items[].signal_id` rows are not written to `/tmp/rt_signal_sent.json`, even when `RT_ALERT_MARK_INELIGIBLE_SENT=1`, so a repaired packet can expose the same alert again. Ordinary safety-gate blocks such as readiness not READY, `eligible_for_approval=false`, or Hermes blocking reasons still follow the existing anti-spam behavior.
