...
The bridge marks emitted alerts or position reviews as sent only after Feishu delivery succeeds. Safety-gate-blocked technical triggers are suppressed by default and marked locally so they do not repeat-spam Feishu. Structural Hermes packet problems such as missing/invalid packets, missing matching `review_items[]`, or duplicate `review_items[].signal_id` are retryable and are not written to the alert sent-state, so a repaired packet can surface the same alert again.
