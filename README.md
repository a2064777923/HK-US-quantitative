...
- Current strategy config is `v5.8-factor-conflict-discipline-20260620`.
...
- v5.8 keeps the guarded momentum breakout model, downgrades noisy or underperforming BUY triggers to diagnostic `WATCH` unless later evidence supports re-enabling, moves configured downgraded SELL diagnostics to summary-only, ranks same-symbol same-scan executable candidates by quality, blocks BUY execution candidates when realtime breadth is missing/risk-off or the symbol's same-session `change_pct` is below the configured realtime-alignment floor, and downgrades directional candidates when too many factor families oppose the candidate side.
...
- v5 enables same-market realtime breadth gating by default: a new BUY candidate is downgraded to WATCH when the scanned market is risk-off, or when the live v5.8 config cannot see enough same-market realtime breadth to validate the context.
...
v5.8 does not globally loosen execution gates. A `急漲` row may carry `candidate_signal_type=BUY`, but it remains `WATCH` unless full-score confirmation, factor confluence, opposing-factor discipline, risk geometry, liquidity, realtime breadth context, realtime direction alignment, and execution-candidate checks pass. `布林上軌動量突破` replaces the old unconditional upper-band SELL interpretation only when same-session or recent momentum supports a breakout context; weak or overbought upper-band touches still remain SELL/WATCH diagnostics.
When a single symbol emits multiple executable BUY or SELL candidates in the same scan, v5.8 keeps the higher-quality candidate and marks lower-ranked same-direction rows as `same_scan_directional_duplicate` diagnostics. Candidates already blocked by the same-session or cooldown dedup state are skipped instead of being re-emitted as WATCH rows.

The live v5.8 config uses `trigger_overrides` plus intraday and factor-conflict discipline gates to reduce weak-market BUY noise and downgraded SELL diagnostic noise:
...
- `BUY:急漲` is tightened with a higher score threshold, longer cooldown, and `large_move_buy_min_non_momentum_factors=2`, so same-session and 5-day momentum cannot self-confirm an executable BUY without at least two non-momentum factor families.
- `confirmation_requirements.*.max_opposing_factor_count=1` downgrades a directional candidate to `WATCH` when more than one structured factor family supports the opposite side; alerts expose `factor_confluence_opposing_categories`, `factor_confluence_opposing_count`, and `factor_confluence_reason=opposing_factor_conflict`.
...
- `market_breadth_model.block_new_buy_when_context_missing=true` downgrades new BUY candidates to `WATCH` when same-market realtime breadth is missing, insufficient, or malformed; Hermes should read `market_breadth_status` and `execution_blocked_reasons` instead of treating missing breadth as neutral support.
...
The bridge marks emitted alerts or position reviews as sent only after Feishu delivery succeeds. Safety-gate-blocked technical triggers are suppressed by default and marked locally so they do not repeat-spam Feishu. Structural Hermes packet problems such as missing/invalid/stale packets, missing matching `review_items[]`, or duplicate `review_items[].signal_id` are retryable and are not written to the alert sent-state, so a repaired packet can surface the same alert again.
...
Position-review notifications default to `RT_POSITION_REVIEW_ROLES=user`, include `high,medium` urgency, cap at 20 items, and remind once per stable holding thread per 24 hours unless urgency or action escalates. Simulation holdings still remain in the Hermes packet and reports, but they are not mixed into the operator Feishu stream unless `RT_POSITION_REVIEW_ROLES=simulation` or `all` is set explicitly. These notifications are titled as `Hermes持倉審核待辦（不下單）`, include `order_submission=false`, and remain advisory-only. The `審核草案` and `動態管理` lines are structured review aids, not order tickets; `px_age` is the age of the DB position price snapshot, and `sig_stop_ref` / `sig_target_ref` are latest-signal geometry references, not broker order prices for the held long position.
...
`system_health_check.py` emits `schema=quantmind_system_health_v1`. `execution_readiness_report.py` treats known-schema `system_health` or `data_health` `WARN` as warning gates, while missing/invalid schemas and `FAIL` remain hard blocks. This keeps degraded context visible to Hermes without conflating it with broken health-report contracts.
...
- Deployed `v5.7-intraday-breadth-and-momentum-discipline-20260619`: current-session quotes still participate in trigger detection, same-session momentum, volume pace, realtime alignment, and breadth, but a new BUY execution candidate now requires usable same-market realtime breadth and `急漲` requires at least two non-momentum supporting factor families before it can remain executable.
...
- Added a dedicated `portfolio_price_snapshot` section to `cron_audit_report.py` so missing user price snapshot coverage warns without misclassifying valuation-maintenance cron as read-only evidence cron.
